...
class _CategoryListPageState extends State<CategoryListPage> {
  ...
  void _onSearch(String q) {
    setState(() {
      ...
      _filtered = _categories.where((c) =>
        ...
        (c['id'] ?? '').toLowerCase().contains(_search)).toList();
    });
  }
          ...
          rows: _filtered.map((c) => [
            ...
            Row(children: [
              IconButton(
                ...
                tooltip: 'Edit',
                onPressed: () => _showFormBottomSheet(context, cat: c),
              ),
              IconButton(
  ...
  void _showFormBottomSheet(BuildContext context, {Map<String, dynamic>? cat}) {
    ...
    final nameCtrl   = TextEditingController(text: isCreate ? '' : cat!['categoryName'] ?? '');
    final descCtrl   = TextEditingController(text: isCreate ? '' : cat!['description'] ?? '');
    bool loading     = false;
        ...
        builder: (ctx, setS) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
              ...
              Text(isCreate ? 'Tambah Kategori' : 'Edit Kategori',
                style: TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 16,
                  fontWeight: FontWeight.w700, color: textColor)),
              const SizedBox(height: 20),

              // ID (read-only)
              Text('ID Kategori', style: TextStyle(fontFamily: AppTheme.fontFamily,
              ...
              Container(
                ...
                child: Text(generatedId, style: const TextStyle(fontFamily: AppTheme.fontFamily,
                  fontSize: 13, fontWeight: FontWeight.w600, color: AppTheme.primary)),
              ),
              const SizedBox(height: 14),

              // Nama Kategori
              Text('Nama Kategori', style: TextStyle(fontFamily: AppTheme.fontFamily,
                fontSize: 12, fontWeight: FontWeight.w500, color: subColor)),
              const SizedBox(height: 6),
              TextField(
                controller: nameCtrl,
              ...
              // Deskripsi
              Text('Deskripsi', style: TextStyle(fontFamily: AppTheme.fontFamily,
                fontSize: 12, fontWeight: FontWeight.w500, color: subColor)),
              const SizedBox(height: 6),
              TextField(
                controller: descCtrl,
                  ...
                  onPressed: loading ? null : () async {
                    ...
                    try {
                      if (isCreate) {
                        ...
                        await RemoteHelper.getDio().put('api/categories/$generatedId', data: {
                          'categoryName': nameCtrl.text,
                          'description': descCtrl.text,
                        });
                      ...
                      setS(() => loading = false);
                      _showSnack(isCreate ? 'Gagal menambahkan!' : 'Gagal mengupdate!', isError: true);
                    }
                  },
                  style: ElevatedButton.styleFrom(
                    backgroundColor: AppTheme.primary, foregroundColor: Colors.white,
